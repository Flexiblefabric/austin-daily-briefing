...
| SEC-1 | Subscriber data security | Document data classification, access-control baseline, retention, incident-response, and periodic access review for subscriber email/preference data | Review | Draft baseline added in `docs/subscriber-data-security-baseline.md`; retention now reconciled to the already-approved `docs/data-retention.md`; owner review remains for classification, access-control, incident-response, and quarterly-review baseline |
| SEC-2 | Subscriber data security | Audit production Drive/Sheet sharing, folder inheritance, editor rights, account MFA, Apps Script access, and external collaborators | Ready | Use the SEC-1 baseline as the audit standard; manual Google/provider review |
