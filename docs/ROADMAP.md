...
| FORM-4 | Native customization | Add subscriber verification for preference changes using single-use, scoped confirmation links and hashed tokens | Ready | SEC-1 security model complete; must also eliminate raw-token persistence identified by SEC-3 before production promotion |
