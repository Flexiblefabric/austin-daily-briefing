...
- For a new response, read the exact production tab's header and populated row. Take its displayed cell strings from the first through the last response column, preserving blank cells between populated columns and excluding trailing unused cells. For Signup these are Timestamp, Email Address, and the consent answer.
